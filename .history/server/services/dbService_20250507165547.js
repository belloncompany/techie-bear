...
const dbService = {
    ...
    async updateIngredientsForOrder(menuItemId, quantity) {
        const ingredients = await this.getMenuItemIngredients(menuItemId);
        ...
        try {
            ...
            for (const ingredient of ingredients) {
                const newQuantity = ingredient.quantity - (ingredient.required_amount * quantity);
                if (newQuantity < 0) {
    ...
    async createOrder(items) {
        ...
        try {
            ...
            // Create order with total amount - now setting initial status as 'processing'
            ...
            for (const item of orderItems) {
                ...
                await this.updateIngredientsForOrder(item.menuItemId, item.quantity);
            }
